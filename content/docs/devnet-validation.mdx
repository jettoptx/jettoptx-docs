...
# Configure validator environment from the template
cp .env.example .env   # then fill in AARON_VALIDATOR_KEY and RPC_URL
...
The validator reads its configuration from a `.env` file. Copy the provided
[`.env.example`](https://github.com/jettoptx/jettoptx-docs/blob/main/.env.example)
template and fill in your own values — never commit a real `.env`:

```bash
AARON_VALIDATOR_KEY=<your-validator-key>
