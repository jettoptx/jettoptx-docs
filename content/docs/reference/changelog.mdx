...
## 2026-06-29 — v2.1.5: Solana-native bridge wording cleanup

### Fixed
- **Architecture Flows index** — attestation bridge blurb matches `/docs/architecture/bridge-flow` (gaze → AARON → Solana PDAs)
- **API Reference** — `bridge_status` WebSocket comment no longer references LayerZero
- **How It Works** — sections 3c/3e (XRP USDC bridge, Xahau hooks) marked **roadmap**; production path remains Solana-native
- **Footer tagline** — "cross-chain bridging" → Solana-native on-chain proofs
- **GROK_BUILD_HANDOFF_PROMPT.md** — internal-only banner (not deployed to site)
