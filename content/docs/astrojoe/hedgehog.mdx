...
> **Chat-safe default:** Jetson HEDGEHOG uses `grok-4.5`. The older multi-agent model id returns HTTP 400 on `/v1/chat/completions` and is remapped server-side.


| Priority | Model | Use Case |
|----------|-------|----------|
| **Primary** | `grok-4.5` | Web search, X search, research, spatial reasoning |
| **Fallback** | `grok-4.5` | Complex reasoning without multi-agent tools |
